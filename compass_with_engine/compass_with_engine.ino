...
int motorPin = 2;
// Декларируем переменную для магнитометра
HMC5883L compass;
...
void setup()
{
  ...
  //Активируем монитор для наблюдения показаний
  Serial.begin(9600);
...
void loop()
{
  ...
  // Если координата Х находится в диапазоне [100,200]
  if(raw.XAxis >= 100 && raw.XAxis <= 200) {
    // Включить мотор
